...
class DisplayCanvas {
    constructor(size, canvasScale = 15) {
        this.size = size;
    ...
    getImage(from = { x: 0, y: 0 }, to = { x: this.size.width, y: this.size.height }, displayGrid = false) {
        const canvasScale = this.canvasScale;
        const size = {
            width: to.x - from.x,
            height: to.y - from.y
        ...
        if (this.isGetImageRangeVaild(from, to)) {
            ...
            ctx.drawImage(this.canvas, from.x * canvasScale, from.y * canvasScale, size.width * canvasScale, size.height * canvasScale, 0, 0, size.width * canvasScale, size.height * canvasScale);
        }
        else {
            canvas = this.canvas;
            ctx = this.ctx;
        ...
        if (displayGrid) {
            ...
            for (let x = 0; x < size.width; x++) {
                ...
                ctx.stroke();
                if ((x === 0 && from.x === from.y) ||
                    (x !== 0 && x % 5 === 0)) {
                    let xCoord = from.x + x + 1;
                    const metrics = ctx.measureText(xCoord.toString());
                    ctx.globalCompositeOperation = "difference";
                    ctx.fillStyle = "#ffffff";
                    ctx.fillText(xCoord.toString(), strokePos + canvasScale / 2 - metrics.width / 2, canvasScale - (metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent) / 2, canvasScale);
                }
            }
            for (let y = 0; y < size.height; y++) {
                ...
                ctx.stroke();
                if ((y === 0 && from.x === from.y) ||
                    (y !== 0 && y % 5 === 0)) {
                    let yCoord = from.y + y + 1;
                    const metrics = ctx.measureText(yCoord.toString());
                    ctx.globalCompositeOperation = "difference";
                    ctx.fillStyle = "#ffffff";
                    ctx.fillText(yCoord.toString(), canvasScale / 2 - metrics.width / 2, strokePos + canvasScale - (metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent) / 2, canvasScale);
                }
            }
        }
